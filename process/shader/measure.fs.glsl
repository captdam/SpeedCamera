...
void main() {
	...
	if (texelFetch(current, pxIdx, 0).r == 1.0) { //==1.0 means edge
		highp float currentPos = texture(roadmapT1, pxPos).CH_ROADMAP1_POS_OY; //Roadmap may have different size, interpolation is OK
...
//				float roadPos = texelFetch(roadmapT1, roadIdx, 0).CH_ROADMAP1_POS_OY;
				...
				highp float roadPos = texture( roadmapT1 , vec2(idx)/videoSizeF ).CH_ROADMAP1_POS_OY;
